...
/* WEBPACK VAR INJECTION */(function($) {/* harmony import */ var _02_homeHtml2Canvas__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./02_homeHtml2Canvas */ "./src/js/02_homeHtml2Canvas.js");
/* harmony import */ var _02_homeHtml2Canvas__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_02_homeHtml2Canvas__WEBPACK_IMPORTED_MODULE_0__);
// jshint ignore: start
...
var avgX = [];
var avgY = [];
var previousCoords = [Math.random() * window.innerWidth, Math.random() * window.innerHeight];
var strokeWidth = 65;
var shapes = 0;
var titles = ['FFF-Corp', '333', 'Design', 'Illustration', 'Direction', 'Layout', 'Typography', 'CGI'];
var palette = ['#FFD600', '#2400FF', '#FF4D00'];
var randomColor = palette[Math.floor(Math.random() * palette.length)]; // Not

document.addEventListener('DOMContentLoaded', function () {// let node = document.querySelector('.preload-transitions');
...
document.title = titles[Math.floor(Math.random() * titles.length)]; // Generate SVG

if (document.getElementById('bauhausShapes') != null) {
  document.getElementById('bauhausShapes').setAttribute('viewBox', '0 0 ' + window.innerWidth + ' ' + window.innerHeight);
  document.getElementById('bauhausShapes').setAttribute('height', window.innerHeight);
  document.getElementById('bauhausShapes').setAttribute('width', window.innerWidth);
...
document.onclick = function (e) {
  if (e.target.tagName != 'A' && e.target.tagName != 'INPUT') {
    ...
    if (previousCoords.length > 0) {
      if (Math.random() <= 0.3) {
        var newShape = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
        var _randomColor = palette[Math.floor(Math.random() * palette.length)];
        ...
        newShape.style.stroke = _randomColor;
        newShape.style.fill = _randomColor;
        ...
        var _randomColor2 = palette[Math.floor(Math.random() * palette.length)];
        newShape.setAttribute('cx', e.clientX);
        newShape.setAttribute('cy', e.clientY);
        newShape.setAttribute('r', strokeWidth);
        newShape.style.strokeWidth = strokeWidth;
        newShape.style.stroke = _randomColor2;
        newShape.style.fill = _randomColor2;
        document.getElementById('bauhausShapes').appendChild(newShape);
      } else {
        var newShape = document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
        var _randomColor3 = palette[Math.floor(Math.random() * palette.length)];
        newShape.setAttribute('points', "".concat(e.clientX + 50, ",").concat(e.clientY + 10, " ").concat(e.clientX + 100, ",").concat(e.clientY + 100, " ").concat(e.clientX + 0, " ,").concat(e.clientY + 100)); // newShape.setAttribute('points', "x1,y1 x2,y2 x3,y4");

        newShape.style.strokeWidth = strokeWidth;
        newShape.style.stroke = _randomColor3;
        newShape.style.fill = _randomColor3;
        document.getElementById('bauhausShapes').appendChild(newShape);
      }
    }
...
function saveImage() {
  ...
  _02_homeHtml2Canvas__WEBPACK_IMPORTED_MODULE_0___default()(document.body, {
    width: w,
    height: h
  }).then(function (canvas) {
    var svg = document.getElementById('bauhausShapes');
    var img = document.querySelector('img'); // get svg data

    var xml = new XMLSerializer().serializeToString(svg); // make it base64
    ...
    img.src = image64; // draw the image onto the canvas
...
var inputs = [].slice.call(document.querySelectorAll('input#blur,input#spacing'));
var inputRotation = document.querySelectorAll('#rotation');
inputRotation.forEach(function (input) {
  return input.addEventListener('change', handleUpdateRotation);
});
inputRotation.forEach(function (input) {
  return input.addEventListener('mousemove', handleUpdateRotation);
});
...
function handleUpdate(e) {
  var suffix = this.id === 'base' ? '' : 'px';
  document.getElementById('bauhausShapes').style.setProperty("--".concat(this.id), this.value + suffix);
}
