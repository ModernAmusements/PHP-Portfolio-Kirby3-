...
isTouch ? (console.log("touch"), document.body.classList.add("touch-device"), lightboxWidth = 1) : document.body.classList.add("not-touch-device");
var vh = .01 * window.innerHeight;
document.documentElement.style.setProperty("--vh", vh + "px"), window.addEventListener("resize", function () {
  vh = .01 * window.innerHeight, document.documentElement.style.setProperty("--vh", vh + "px")
});
...
function showIntro() {
  1 == introFlag ? (document.querySelector(".intro") && (setTimeout(function () {
...
function handlePosts(e) {
  ...
  t && t.addEventListener("click", function () {
    ...
    r.forEach(function (e, t) {
      e.children[0].children[0].offsetWidth >= e.children[0].offsetWidth && e.classList.add("")
    }) // show all kat.
  }), n && r && (Array.from(n.querySelectorAll(".category-button")).forEach(function (e) {
    e.addEventListener("click", function () {
      n.querySelector(".category-li.active").classList.remove("active"), e.parentElement.classList.add("active");
      var t = e.dataset.slug;
      r.forEach(function (e) {
        e.dataset.categories.split(" ").filter(function (e) {
          return "" != e
        }).some(function (e) {
          return t == e
        }) ? e.classList.remove("hide") : (e.classList.add("hide"), e.classList.remove("toggle"), e.removeAttribute("style"))
      })
    })
    ...
    i.children[0].addEventListener("mouseenter", function (e) {
      ...
      i.children[0].children[2] && i.children[0].children[2].removeAttribute("style")
    }), i.children[0].addEventListener("click", function () { // close tab remove all style hide !
      if (!i.classList.contains("toggle") && (i.classList.contains("with-video") && i.classList.contains("video-loaded") || i.classList.contains("with-slider") && i.classList.contains("first-img-loaded"))) {
        if (!i.classList.contains("imgs-loaded")) Array.from(i.querySelectorAll("img:not(.active)")).forEach(function (e) {
          e.setAttribute("src", e.dataset.src), e.removeAttribute("data-src")
        }), i.classList.add("imgs-loaded");
        ...
        i.dataset.height = i.children[1].offsetHeight + 50 + "px", i.classList.add("toggle"), i.style.height = i.dataset.height
    ...
    if (t && t.addEventListener("load", function () {
...
var MRTransition = Barba.BaseTransition.extend({
  start: function () {
    document.body.classList.add("loading-initialized"), document.body.classList.add("transition-started"), Promise.all([this.newContainerLoading, this.minimalTimeout()]).then(this.finish.bind(this))
  },
  ...
  finish: function () {
    ...
    window.scrollTo(0, 0), document.body.classList.remove("loading-initialized"), setTimeout(function () {
